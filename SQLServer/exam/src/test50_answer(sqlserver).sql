...
select a.c_id,b.c_name,MAX(s_score) as 最高分,MIN(s_score) as 最低分,ROUND(AVG(s_score),2) as 平均分,
    cast((select count(1) from Score where C_ID = a.C_ID and s_score >= 60)*100.0 / (select count(1) from Score where C_ID = a.C_ID) as decimal(18,2)) 及格率 ,
    cast((select count(1) from Score where C_ID = a.C_ID and s_score >= 70 and s_score < 80 )*100.0 / (select count(1) from Score where C_ID = a.C_ID) as decimal(18,2)) 中等率 ,
	cast((select count(1) from Score where C_ID = a.C_ID and s_score >= 80 and s_score < 90 )*100.0 / (select count(1) from score where C_ID = a.C_ID) as decimal(18,2)) 优良率 ,
	cast((select count(1) from score where C_ID = a.C_ID and s_score >= 90)*100.0 / (select count(1) from score where C_ID = a.C_ID) as decimal(18,2)) 优秀率 
